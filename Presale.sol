...
contract Presale is Ownable {
    ...
    function updateTester(address _addr, bool _status) public onlyOwner {
        tester[_addr] = _status;
    }
    ...
    mapping (uint256 => uint256)  public address_amount;
    mapping (address => uint256[]) remains_time;
    mapping (address => uint256[]) remains_amount;
    function buyToken(uint256 _amount) public payable {
        if(!on_sale) {
            require(tester[tx.origin], "Pre-sales period has ended");
        }
        
        cost = _amount.mul(price).div(token_out_unit);
        require(cost >= min_buy, "The amount smaller than min buy amount");
        require(cost <= max_buy, "The amount bigger than max buy amount");
        require(token_out.balanceOf(sender) >= _amount, "Pre-sales program has ended");
        
        // Send first amount
        token_in.transferFrom(tx.origin, sender, cost);
        ...
        address_amount[index] = _amount;
    }
    
    // Check amount
    function remainToken(address _addr) public view returns (uint256, uint256, uint256, uint256) {
        uint256 available_amount = 0;
        uint256 next_time = 99999999999999999999999;
        uint256 next_amount = 0;
        uint256 total_remain = 0;
        
        for(uint256 i=0; i<remains_time[_addr].length; i++){
            if(remains_time[_addr][i] <= block.timestamp) {
                // Get available amount can take
                if(remains_amount[_addr][i] != 0) {
                    available_amount = available_amount.add(remains_amount[_addr][i]);
                }
            }
            else {
                // Get next time and next amount can get
                if(next_time > remains_time[_addr][i] && remains_amount[_addr][i] != 0) {
                    next_time = remains_time[_addr][i];
                    next_amount = remains_amount[_addr][i];
                }
                // Get total remain
                total_remain = total_remain.add(remains_amount[_addr][i]);
            }
        }
